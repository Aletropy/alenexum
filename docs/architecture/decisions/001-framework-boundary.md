...
description: Why Nexum composes discord.js instead of reimplementing it.
---
...
- **Decision:** discord.js / `@discordjs/*` own all network infrastructure. Nexum owns application architecture only, integrated through thin seams (`Connector`, `observer`, `tracer`, escape hatches).
...
- **Consequences:** Apps depend on discord.js directly and can use any discord.js feature immediately via escape hatches. Nexum must never shadow discord.js semantics; new framework features require a "why not discord.js?" justification.
